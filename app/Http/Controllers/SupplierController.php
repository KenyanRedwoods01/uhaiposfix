...
class SupplierController extends Controller
{
    ...
    public function index()
    {
        ...
        if ($role->hasPermissionTo('suppliers-index')) {
            ...
            // Only fetch active suppliers for this tenant
    ...
    public function clearDue(Request $request)
    {
        ...
        if (!$supplier) {
            return redirect()->back()->with('not_permitted', 'Supplier not found or not accessible');
        }

        // Fetch due purchases for this supplier
        ...
        foreach ($lims_due_purchase_data as $purchase_data) {
            if ($total_paid_amount == 0) break;

            $due_amount = $purchase_data->grand_total - $purchase_data->paid_amount;

            // Look up cash register for this user + warehouse
    ...
    public function store(Request $request)
    {
        ...
        // Extra validation if creating both supplier + customer
        ...
        $lims_supplier_data['pos_accnt_id'] = $posAccntId;
        ...
        $mail_setting = MailSetting::where('pos_accnt_id', $posAccntId)->latest()->first();
    ...
    public function deleteBySelection(Request $request)
    {
        ...
        foreach ($request['supplierIdArray'] as $id) {
            $lims_supplier_data = Supplier::where('id', $id)
                                          ->where('pos_accnt_id', $posAccntId)
                                          ->first();
    ...
    public function importSupplier(Request $request)
    {
        ...
        while ($columns = fgetcsv($file)) {
            ...
            $supplier->pos_accnt_id = $posAccntId;
            $supplier->save();
